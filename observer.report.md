...
- Run: `D:\works\lr-wp-ai-agent\runs\motorcycle-chair-fix`
- Generated: 2026-07-22 14:01:14
- Status: **NEEDS WORK**
- Errors: 0
- Warnings: 1
...
| resolved blocks | `pass` | 13 blocks |
...
| missing block report | `skip` | not present |
...
| manual-fill.md | `warn` | 6 open items |
| content placeholder scan | `pass` | no placeholder patterns found |
| screenshots | `pass` | desktop=1, mobile=1 |
| dev server responds | `skip` | pass -DevUrl to check |
...
| --- | --- | --- | --- | --- |
| `warning` | user-data | manual-fill.md has 6 open user-data items. | manual-fill.md | Collect real user data or explicitly accept the fallback before publishing. |
...
- Re-run `D:\works\lr-wp-ai-agent\scripts\validate-run.ps1 -RunPath "D:\works\lr-wp-ai-agent\runs\motorcycle-chair-fix"` after changes.
