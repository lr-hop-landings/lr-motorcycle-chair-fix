import { createQuizTransition } from './quiz-transition.js';
...
const app = hopRoot.querySelector('#quiz-app'), dialog = hopRoot.querySelector('#contact-dialog');
...
let analysisTimers = [];
const transitionStep = createQuizTransition(app, clearAnalysis);
...
function render(focus = true) {
    ...
    app.querySelector('[data-back]')?.addEventListener('click', () => transitionStep(() => { state.step = state.step >= 4 ? 3 : questionSteps[Math.max(0, questionSteps.indexOf(state.step) - 1)]; render(); }));
    app.querySelector('[data-reset]')?.addEventListener('click', () => transitionStep(() => { Object.assign(state, initialState); render(); }));
    app.querySelector('[data-prepare]')?.addEventListener('click', openContact);
    setupLeadForm();
    if (focus) {
        hopRoot.querySelector('#question-title').focus({ preventScroll: true });
        hopRoot.querySelector('#quiz').scrollIntoView({ block: 'start', behavior: matchMedia('(prefers-reduced-motion: reduce)').matches ? 'instant' : 'smooth' });
    }
}
...
function startAnalysis() {
    ...
    analysisTimers.push(window.setTimeout(() => {
        transitionStep(() => { state.step = 5; render(); });
    }, 1800));
}
...
window.addEventListener('submit', event => {
    ...
    if (!form.matches?.('.quiz-lead-form, .callback-form') || !hopRoot.contains(form) || event.defaultPrevented) return;
    event.preventDefault();
...
function openContact(event) { event?.preventDefault(); contactTrigger = document.activeElement; hopRoot.querySelector('#message').value = messageText(); hopRoot.querySelector('#copy-status').textContent = ''; previousOverflow = document.body.style.overflow; dialog.showModal(); document.body.style.overflow = 'hidden'; dialog.querySelector('.close').focus(); }
...
hopRoot.querySelectorAll('[data-contact]').forEach(a => a.addEventListener('click', openContact));
hopRoot.querySelectorAll('[data-intent]').forEach(a => a.addEventListener('click', e => { e.preventDefault(); transitionStep(() => { if (state.intent !== a.dataset.intent)
    state.detail = null; state.intent = a.dataset.intent; state.step = 1; render(); }, null, 120); }));
hopRoot.querySelectorAll('a[href="#quiz"], a[href="#estimate"]').forEach(link => link.addEventListener('click', event => {
...
hopRoot.querySelector('#copy-message').addEventListener('click', async () => { const msg = hopRoot.querySelector('#message'); try {
    await navigator.clipboard.writeText(msg.value);
    hopRoot.querySelector('#copy-status').textContent = 'Текст скопирован. Откройте мессенджер и добавьте фотографии.';
}
catch (e) {
    msg.focus();
    msg.select();
    hopRoot.querySelector('#copy-status').textContent = 'Не удалось скопировать. Выделите текст и скопируйте вручную.';
} });
render(false);
